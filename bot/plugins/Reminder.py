...
class Reminder(Cog):
    ...
    async def _reminder_loop(self):
        ...
        reminders = await self.bot.database.fetch("SELECT * FROM reminder WHERE due < $1", due_before)
    ...
    async def _execute_reminder(self, now: datetime.datetime, reminder: asyncpg.protocol.Record) -> None:
        await self.bot.wait_until_ready()
        sleep = (reminder['due'] - now).seconds
        
        await asyncio.sleep(sleep)
        
        channel: typing.Optional[typing.Union[discord.TextChannel, discord.VoiceChannel]] = await self.bot.fetch_channel(reminder['channel'])
    ...
    async def _reminder_delete(self, interaction: discord.Interaction, id: int) -> None:
        ...
        """
        await interaction.response.defer(thinking=True, ephemeral=True)
        r = await self.bot.database.fetchrow("DELETE FROM reminder WHERE id = $1 AND member = $2 RETURNING *", id, interaction.user.id)
        if not r:
            raise ReminderDoesntExist
        else:
            await interaction.followup.send(embed=SuccessEmbed(f"Deleted reminder {r['id']} ({r['reason']})"), ephemeral=True)
    ...
    async def _reminder_list(self, interaction: discord.Interaction) -> None:
        ...
        """
        await interaction.response.defer(thinking=True, ephemeral=True)
        reminders = await self.bot.database.fetch("SELECT id, reason, due FROM reminder WHERE member = $1", interaction.user.id)
        if not reminders:
            raise NoReminders
        else:
            embed = NeutralEmbed(
